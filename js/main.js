...
function burgerOpen() {
  ...
  site.classList.add('stop-scroll');
}
...
document.addEventListener('click', function (e) {
  ...
  const clickOnBurger = burgerBtn.contains(e.target);
...
menuList.forEach(link => {
  link.addEventListener('click', function () {
    menuList.forEach(item => item.classList.remove('nav__link--active'));
...
//Link to slider
const reviewLinks = document.querySelectorAll('.step__wrapper');
...
reviewLinks.forEach(link => {
  link.addEventListener('click', e => {
    ...
    setTimeout(() => {
      reviewsSlider.slideToLoop(slideIndex, 600);
    }, 400);
  });
});
//FORM INSIDE POPUP ADD INFORMATION
const productInnput = document.querySelector('#product-name');
const priceInput = document.querySelector('#product-price');
const sliderButtons = document.querySelectorAll('.reviews__button');
sliderButtons.forEach(btn => {
  btn.addEventListener('click', function (event) {
    ...
    const productName = slide.querySelector('.review-card__name').textContent;
    productInnput.textContent = productName;
...
//TG messages
const TG_TOKEN = '8275362352:AAHm-d9pqaMupct6Y56UkvvvZx_V3TgVqIY';
const TG_CHAT_ID = '-5229755369';
function sendToTelegram() {
  ...
  console.log(nameInput, emailInput, productName, productPrice);
  const text = `
📩 Новая заявка с сайта

👤 Имя: ${nameInput.value}
📧 Email: ${emailInput.value}
📦 Продукт: ${productName.textContent}
💰 Тариф: ${productPrice.textContent}
  `;
...
const consentCheckbox = form.querySelector('#ch1');
